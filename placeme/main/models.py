...
# ---  KEEP THESE (They might be needed for Django internals) ---
...
class AuthUser(models.Model): # Potentially redundant, but keep for now
...
# RENAMED: PlacementOffer -> Placement,  Application -> StudentApplication
class Placement(models.Model):  # Renamed for clarity
    ...
    liked_by = models.ManyToManyField(User, related_name='liked_placements', blank=True) # ADD THIS

    def __str__(self):
        return self.company_name

class StudentApplication(models.Model): # Renamed for clarity
    """Application Table"""
    ...
    offer_id = models.ForeignKey(Placement, on_delete=models.CASCADE, related_name="applications")  # Relation with PlacementOffer table
    status = models.CharField(max_length=50, default="Applied")  # e.g., "Applied", "Shortlisted", "Rejected"
    applied_date = models.DateField(auto_now_add=True) # Automatically set when created

    class Meta:
        unique_together = ('reg_no', 'offer_id') # Prevent duplicate applications!

    def __str__(self):
        return f"Application {self.appli_id} - {self.reg_no}"

class Notification(models.Model): # ADD THIS
